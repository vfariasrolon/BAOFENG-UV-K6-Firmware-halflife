...
/* Standard 5x7 ASCII Font (space to ~) */
...
void LCD_DrawChar(U8 x, U8 y, char c, U8 scale)
{
    ...
    /* Cada caracter tiene 5 columnas de ancho + 1 de espacio. 
       Su alto es 7 bits + 1 espacio abajo. */
    for (col = 0; col < 5; col++) {
        bits = Font5x7[c - 32][col];
        for (row = 0; row < 7; row++) {
            if (bits & (1 << row)) {
                // Dibujar pixel considerando la escala
                for (sx = 0; sx < scale; sx++) {
                    for (sy = 0; sy < scale; sy++) {
                        LCD_DrawPixel(x + (col * scale) + sx, y + (row * scale) + sy, 1);
                    }
                }
            }
        }
